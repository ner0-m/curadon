...
class device_volume_nd {
  public:
    device_volume_nd(T *data, vec<u64, Dim> shape)
        : device_volume_nd<T, Dim>(data, shape, vec<f32, Dim>::ones()) {}

    device_volume_nd(T *data, vec<u64, Dim> shape, vec<f32, Dim> spacing)
        : device_volume_nd<T, Dim>(data, shape, vec<f32, Dim>::ones(), vec<f32, Dim>::zeros()) {}

    device_volume_nd(T *data, vec<u64, Dim> shape, vec<f32, Dim> spacing, vec<f32, Dim> offset)
        : data_(data, shape)
        , spacing_(spacing)
    ...
    device_span_2d<T> kernel_span() {
        return device_span_2d<T>(data_.device_data(), data_.shape(), data_.strides());
    }
